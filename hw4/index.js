...
function settexField(text, predict) {
...
function prepUI(predict) {
    ...
    testExampleSelect.addEventListener('change', () => {
        settexField(examples[testExampleSelect.value], predict);
    });
    settexField(example['example1'], predict);
}
...
async function loadHostedPretrainedModel(url) {
    ...
    try {
        const model = await tf.loadLayerModel(url);
...
class Classifier {
    ...
    predict(text) {
        // Conver to lower case and remove all punctations.
